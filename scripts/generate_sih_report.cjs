...
console.log('Generating comprehensive SIH Project Report for AARAMBHA...');

// Helper to format screenshots
const imgPrefix = 'docs/screenshots/';

const reportContent = `# AARAMBHA — PROCUREMENT INTELLIGENCE PLATFORM
...
const targetPath = path.join(__dirname, '..', 'SIH_Project_Report_AARAMBHA.md');
fs.writeFileSync(targetPath, reportContent, 'utf8');

console.log(`Successfully generated SIH Project Report at: ${targetPath}`);
console.log(`Report word count: ~${reportContent.split(/\\s+/).length} words`);
console.log(`Report character length: ${reportContent.length} characters`);
